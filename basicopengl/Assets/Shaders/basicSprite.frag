...
{
    //float alpha = texture(sceneTexture, TexCoords).a;
    vec4 hdrColor = texture(sceneTexture, TexCoords);      
    vec4 bloomColor = texture(myTexture1, TexCoords);
    if(bloom)
        hdrColor += bloomColor; // additive blending
    // tone mapping
...
if (Pass == 0)
    ...
    //frag_color = vec4(1.0,0.0,0.0,1.0);//texture(myTexture1, TexCoords);
